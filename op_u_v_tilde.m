% OP_U_V: assemble the mass matrix in the parametric domain M = [m(i,j)], m(i,j) = (mu u_j, v_i).
%
...
function varargout = op_u_v_tilde (spu, spv, msh)
...
if (~isempty (msh.jacdet))
    jacdet_weights = msh.quad_weights;
else
    jacdet_weights = [];
end
